...
static bool TestJsonCommentBlockUtf8Valid(void)
{
	tJsonElement Root;
	tJsonParse Parse;
	tJsonFormat Format;
	...
	JsonFormatSetUpIndent(&Format, 0, json_CommentBlock, &Root);
...
static bool TestJsonCommentBlockUtf8Invalid(void)
{
	tJsonElement Root;
	tJsonParse Parse;
	tJsonCharacter Character;
	...
	for (Character = 0x80; ok && (Character < 0x110000); Character++)
	{
		...
		for (InvalidUnit = 0xC0; ok && (InvalidUnit != 0x00); InvalidUnit++)
		{
			...
			for (n = 0; ok && (n < Length - 1); n++)
			{
				ok = ok && (JsonParse(&Parse, JsonUtf8CodeGetUnit(Code, n)) == JSON_PARSE_INCOMPLETE);
			}
...
static bool TestJsonCommentBlockUtf8OutOfRange(void)
{
	tJsonElement Root;
	tJsonParse Parse;
	tJsonCharacter Character;
...
static bool TestJsonCommentBlockUtf8Overlong(void)
{
	tJsonElement Root;
	tJsonParse Parse;
	tJsonCharacter Character;
...
static const tTestCase TestCaseJsonCommentBlockUtf8[] =
{
   { "Valid",      TestJsonCommentBlockUtf8Valid      },
	{ "Invalid",    TestJsonCommentBlockUtf8Invalid    },
	{ "OutOfRange", TestJsonCommentBlockUtf8OutOfRange },
	{ "Overlong",   TestJsonCommentBlockUtf8Overlong   }
};


const tTest TestJsonCommentBlockUtf8 =
{
    "JsonCommentBlockUtf8",
    TestCaseJsonCommentBlockUtf8,
    sizeof(TestCaseJsonCommentBlockUtf8) / sizeof(TestCaseJsonCommentBlockUtf8[0])
};
